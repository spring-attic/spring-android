...
public class OkHttpRequestFactory implements ClientHttpRequestFactory {
     ...
     * recommended to change this property to {@code false}, so as not to run out of memory. This
     * will result in a {@link ClientHttpRequest} that either streams directly to the underlying
     * {@link HttpURLConnection} (if the
     * {@link org.springframework.http.HttpHeaders#getContentLength() Content-Length} is known in
     * advance), or that will use "Chunked transfer encoding" (if the {@code Content-Length} is not
     * known in advance).
     ...
     * {@link org.springframework.http.HttpHeaders#getContentLength() Content-Length} is not known
     * in advance.
     ...
     * <p>
     * The default implementation uses the given {@linkplain #setProxy(java.net.Proxy) proxy} - if
     ...
     * <p>
     * The default implementation prepares the connection for input and output, and sets the HTTP
     * method.
     * @param connection
